...
class TransactionSet:
	...
	def serialize_service(
		...
		claimseg = claim.claim.segment.split('*')

		datum = {
			'patient_control_number': claimseg[7],
			...
			'payer_city': payer.location.city,
			'payer_state': payer.location.state,
			'payer_zipcode': payer.location.zip_code,
			'payee_name': payee.organization.name if payee.organization else None,
			'payee_city': payee.location.city if payee.location else None,
			'payee_state': payee.location.state if payee.location else None,
			'payee_zipcode': payee.location.zip_code if payee.location else None,
			'payee_npi': payee.organization.identification_code if payee.organization else None,
			'patient_id': claim.entities[0].identification_code,
			'patient_firstname': claim.entities[0].first_name,
			'patient_lastname': claim.entities[0].last_name,
		}
